...
export default class Productions {
    ...
    async getPage (page = 1) {
        try {
            ...
            const response = await ProductionService.get(page , this.params)
            const data = response.data
    ...
    async postProduction (Options, products) {
        try {
            const request_data = {
            ...
            const response = await ProductionService.post(request_data)
            const data = response.data
            return true
        } catch (e) {
            console.log(e)

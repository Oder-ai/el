...
import cl from './FromTableProductChange.module.css'
...
const FromTableProductChange = ({changeModal}) => {
    ...
    const product_color = useInput(ProductionItems.current_item.product_color, {isEmpty: true})
    const product_qty = useInput(ProductionItems.current_item.pairs, {isEmpty: true})
    const package_qty = useInput(ProductionItems.current_item.packages, {})
    const bag_qty = useInput(ProductionItems.current_item.bags, {})
    const defect_worker = useInput(ProductionItems.current_item.defect_worker, {})
    const defect_worker_sum = useInput(ProductionItems.current_item.defect_worker_sum, {})
    const defect_machine = useInput(ProductionItems.current_item.defect_machine, {})
    const defect_machine_sum = useInput(ProductionItems.current_item.defect_machine_sum, {})
    const defect_mehmed = useInput(ProductionItems.current_item.defect_mehmed, {})
    const defect_mehmed_sum = useInput(ProductionItems.current_item.defect_mehmed_sum, {})
                ...
                <div style={{display: 'flex', justifyContent: 'center'}}>
                    <Button
                        ...
                        onClick={async () => {
                            formNotValid()
                            if (formValid) {
                                const pk = ProductionItems.current_item.product
                                const id = ProductionItems.current_item.id
                                await ProductionService.put(id, {
                                    product: pk,
                                    created_at: ProductionItems.current_item.created_at,
                                    color: product_color.value.id,
                                    product_article: ProductionItems.current_item.product_article,
...
export default FromTableProductChange;
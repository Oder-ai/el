...
import cl from './AuthInputSelect.module.css'
import ActionInput from "../ActionInput/ActionInput";
import Select from "../../Svg/Select";
import {Context} from "../../../../index";

const AuthInputSelect = ({value,onChange, onSelect, onBlur, isDirty, marginTop, fontSize, fontWeight, header, errors, ...props}) => {
            ...
            <div style={{display: 'flex', position: 'relative'}}>
                <div className={cl.order_select}>
                    ...
                    {categoryActive ?
                        ...
                        >
                            {Products.current_product_instance.product_colors.map((color, index, list) =>
                                <div
                                    key={color.id}
                                    onClick={() => {
                                        onSelect({color: color.color, pk: color.pk})
                                        setCategoryActive(false)
                                    }}
                                    className={(() => {
                                        if (index === 0) {
